    ...
    internal class NpmScriptRunner
    {
        ...
        public void AttachToLogger(ILogger logger)
        {
            ...
            StdErr.OnReceivedLine += line =>
            {
                if (!string.IsNullOrWhiteSpace(line))
                {
                    string plainLine = StripAnsiColors(line);
                    // Webpack being webpack, it treats error stream as a plain diagnostic stream
                    if (plainLine.Contains("[webpack.Progress]"))
                    {
                        logger.LogInformation(plainLine);
                        Debug.WriteLine(plainLine);
                    }
                    else 
                    {
                        logger.LogError(plainLine);
                    }
                }
            };
